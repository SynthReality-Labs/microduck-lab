...
export interface ComposedJob {
  ...
  /**
   * A prompt to hand to the agent on the training machine.
   *
   * The handoff is agent-to-agent, not human-to-terminal: you paste this into
   * Claude Code or Codex on the box with the GPU and it clones, syncs, smoke
   * tests, trains and exports. That is what makes "resume training elsewhere"
   * a continuation of the same conversation rather than a context switch into
   * shell archaeology.
   */
  agentPrompt: string
}
...
export function composeJob(r: Recipe): ComposedJob {
  ...
  const agentPrompt = [
    `Train a Microduck policy called "${r.behaviour}" for me.`,
    '',
    '1. Clone https://github.com/pollen-robotics/microduck_rl if you do not have it, then `uv sync`.',
    `2. Pre-flight — Pollen reckon 64 envs x 5 iterations catches ~95% of config errors:`,
    '',
    '   ' + smoke.replace(/\n/g, '\n   '),
    '',
    '3. If that looks healthy, run the real thing:',
    '',
    '   ' + command.replace(/\n/g, '\n   '),
    '',
    `4. Watch the reward curve. Every \`Episode_Reward/<penalty>\` must stay <= 0 — a penalty`,
    '   paying out means a sign error, and the policy will farm it.',
    '5. When it converges, export to ONNX and give me the file path:',
    '',
    `   uv run scripts/export.py ${r.task} --wandb-run-path <entity/project/run_id>`,
    '',
    'Then I will drop the .onnx back into MicroDuck Lab and A/B it against the baseline',
    'over the same scenarios and seeds.',
  ].join('\n')

  return { command, target: r.target, estimate, notes, warnings, smokeTestFirst: smoke, agentPrompt }
}
...
export function bundleFiles(r: Recipe, job: ComposedJob): { name: string; content: string }[] {
  return [
    ...
    { name: 'AGENT_PROMPT.txt', content: job.agentPrompt + '\n' },
    {
      name: 'README.md',
      content: [
        `# ${r.behaviour}`,
